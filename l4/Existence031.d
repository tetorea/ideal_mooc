...
public: 
	...
	T[] anticipate(T)(){	// Anticipation031
		debug writeln("Exist031:anticipate 1");
		T[] anticipations = getDefaultAnticipations!T(); 	// list of each Experiment initialized with a proclivity 0
		debug writeln("Exist031:anticipate 2");
		
		if( getEnactedInteraction!Interaction030() !is null ){
			debug writeln("Exist031:anticipate 3");
			foreach( activatedInteraction ; getActivatedInteractions() ){
				debug writeln("Exist031:anticipate 4");
				T proposition = new T( 	(cast(Interaction031)activatedInteraction).getPostInteraction().getExperience(), 
										(cast(Interaction031)activatedInteraction).getWeight() * (cast(Interaction031)activatedInteraction).getPostInteraction().getValence() );
				debug writeln("Exist031:anticipate 5");
				int index = indexOfExp( proposition, anticipations );
				debug writeln("Exist031:anticipate 6");
				if( index < 0 ) anticipations ~= proposition;
				else (anticipations[index]).addProclivity((cast(Interaction031)activatedInteraction).getWeight() * (cast(Interaction031)activatedInteraction).getPostInteraction().getValence());
				debug writeln("Exist031:anticipate 7");
			}
		}
...
protected:	
	...
	/**
	 * create a list of Anticipation containing all Experiment with a proclivity of 0
	 * @return the list of anticipations
	 */

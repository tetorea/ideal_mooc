...
public:	
	...
	override string step() {
		...
		addOrGetPrimitiveInteraction!Interaction010(experience, result);
...
protected:
	 ...
	 */
	T addOrGetPrimitiveInteraction(T)(Experiment experience, Result result) {
		T interaction = addOrGetInteraction!T(experience.getLabel() ~ result.getLabel()); 
	 ...
	 */
	T addOrGetInteraction(T)(string label) {
		if( label !in INTERACTIONS )
			INTERACTIONS[label] = createInteraction!T( label );
		return cast(T)INTERACTIONS[label];
	}
